...
export function buildRevisionPrompt(input: RevisionInput): string {
  ...
  let productDetails = 'TIDAK ADA PRODUK KHUSUS (Konten Organik / Edukasi / Branding)';
  if (product) {
...
TUGAS KAMU:
Revisi thread di atas sesuai instruksi pengguna dengan standar copywriting "ecommerce-copy-humanizer-id".
1. Pastikan bahasa Indonesia yang digunakan santai, natural, gaul, dan relate (e.g. gess, sat-set, boncos, nugas, worth it), tidak kaku.
...
You are Hermes AI Agent, the official AI Marketing & Copywriting Engine for Threads (Meta).
You operate following the /ecommerce-copy-humanizer-id copywriting standard.

CORE RULES:
1. Speak natural, conversational Indonesian with authentic slang (gess, sat-set, boncos, worth it, nugas, anti-ribet, kantong jebol). Never use stiff machine-translated language.
2. Hard Limit: Every single post MUST be strictly UNDER 500 characters.
3. Thread Structure:
   - Post 1: Hook (engaging curiosity gap, problem, curhat relate) + "🧵👇".
   - Post 2: Value / Solution / USP proof / Price packages.
   - Post 3: Actionable CTA with handle @username.
4. Output MUST be ONLY valid JSON matching the requested schema. No conversational filler before or after the JSON.
`.trim();
...
export async function callHermesChatCompletion(
  prompt: string,
  systemPrompt: string = HERMES_SYSTEM_PROMPT
): Promise<string> {
